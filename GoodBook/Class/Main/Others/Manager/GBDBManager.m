...
#import "GBDBManager.h"
#import <FMDatabase.h>

@interface GBDBManager ()
{
    FMDatabase *_fmdb; // 数据库对象
    NSLock     *_lock; // 锁
}
@end
static GBDBManager *_db;
@implementation GBDBManager
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        // 创建数据库
        NSString *dbPath = [NSHomeDirectory() stringByAppendingString:@"/Documents/GoodBookDB.db"];
        NSLog(@"沙盒路径:%@", dbPath);
...
- (NSArray *)selectAllData {
    NSString *sql = @"select * from GoodBook";
